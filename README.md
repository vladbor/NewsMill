...
# Замечание о пороекте для менторов.

1. RabbitMQ в качестве брокера сообщения выбрал потому, что NATS я не знаю совсем, а Redis - это скорее KV-хранилище, а никак не брокер сообщений. Ну и тема для меня новая.
2. Я использовал CLine. Plan/Act - реально хорошая штука:
- при использовании дорогой/дешёвой модели реально позволяет экономить дорогие токены на этапе реализации плана;
- этап планирования даёт возможность заранее оценить то, что напланировал железный разум и устранить замеченные ошибки ещё до начала реализации, а не подпрыгивать от удивления, увидев какую-то предлагаемую для утверждения ересь.
- при использовании бесплатных моделей имеем два лимита вызовов: на планирование и на действия. Полезно.
 Что не понравилось:
 - правила в .clinerules отнюдь не закон и нарушаются регулярно. Например, про то, чтобы брать зрачения всяких там логинов/паролей из переменных окружения (.env),  а не хардкодить их, мне приходилось постоянно писать в промпте;
 - удивительно плохо работали топовые модели от Antropic/OpenAI/Google. Может я не умею их готовить, но на этапе планирования их рассуждения либо сваливались в ошибку, лидо зацикливались, либо советовали всякую ересь (Gemini). Правда я использовал российский API Proxy...